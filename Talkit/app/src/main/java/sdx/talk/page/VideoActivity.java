...
import sdx.talk.R;
import sdx.talk.adapter.PeerConnectionAdapter;
import sdx.talk.adapter.SdpAdapter;
import sdx.talk.bean.NetBean;
import sdx.talk.entity.Constants;
import sdx.talk.util.GsonUtil;
import sdx.talk.net.RtcWebSokcetHelper;
...
//    private EditText etCallUserId;//呼叫userId
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_call);
         ToId = getIntent().getStringExtra(Constants.YOU_ID);
        FromId = getIntent().getStringExtra(Constants.MY_ID);
        initView();
        checkPermission();
    }
...
//                .setAudioDeviceModule(audioDeviceModule)
        ...
        //连接websocket服务
        RtcWebSokcetHelper.getInstance().connectServer(Constants.CAMERA_IP, new RtcWebSokcetHelper.OnReciveServerMgsListener() {
            @Override
            public void onConnect() {
                Log.e(TAG, "onConnect: ");
                Toast.makeText(VideoActivity.this, "服务连接成功", Toast.LENGTH_SHORT).show();
            }
            ...
            public void onRecive(String json) {
                ...
                try {
                    ...
                    switch (code) {
                        case Constants.SendMsg_Code://收到转发消息
                            ...
                            if (sendMsgRequestBean.message.contains("不在线")) {
                                Toast.makeText(VideoActivity.this, "目标用户不在线", Toast.LENGTH_SHORT).show();
                                return;
                            }
                            int turnCode = new JSONObject(sendMsgRequestBean.message).getInt("code");
                            switch (turnCode) {
                                case Constants.Message_Call_Code:
                                    ...
                                    showCustomeDialog("是否同意接听 " + sendMsgRequestBean.fromUserId + " 的来电?",
                                            "同意", new View.OnClickListener() {
                                                ...
                                                public void onClick(View v) {
                                                    NetBean.SendMsgResponseBean sendMsgResponseBean = new NetBean.SendMsgResponseBean(Constants.Message_Call_Result_Code, sendMsgRequestBean.sequenceId, sendMsgRequestBean.toUserId, sendMsgRequestBean.fromUserId, true, "同意接听");
                                                    NetBean.SendMsgRequestBean sendMsgRequestBean1 = new NetBean.SendMsgRequestBean(Constants.SendMsg_Code, sendMsgRequestBean.sequenceId, sendMsgRequestBean.toUserId, sendMsgRequestBean.fromUserId, GsonUtil.BeanToJson(sendMsgResponseBean));
                                                ...
                                                @Override
                                                public void onClick(View v) {
                                                    NetBean.SendMsgResponseBean sendMsgResponseBean = new NetBean.SendMsgResponseBean(Constants.Message_Call_Result_Code, sendMsgRequestBean.sequenceId, sendMsgRequestBean.toUserId, sendMsgRequestBean.fromUserId, false, "拒绝接听");
                                                    NetBean.SendMsgRequestBean sendMsgRequestBean1 = new NetBean.SendMsgRequestBean(Constants.SendMsg_Code, sendMsgRequestBean.sequenceId, sendMsgRequestBean.toUserId, sendMsgRequestBean.fromUserId, GsonUtil.BeanToJson(sendMsgResponseBean));
                                                    RtcWebSokcetHelper.getInstance().sendRequest(GsonUtil.BeanToJson(sendMsgRequestBean1));
                                                }
                                            });
                                    break;
                                case Constants.Message_SDP_Offer_Code:
                                    ...
                                    break;
                                case Constants.Message_SDP_Answer_Code:
                                    ...
                                    break;
                                case Constants.Message_IceCandidate_Request_Code:
                                    ...
                                    break;
                                case Constants.Message_IceCandidate_Response_Code:
                            ...
                            callBackEvent(json);
                            break;
                        case Constants.Login_Code://登录通知
                            callBackEvent(json);
                            break;
                        case Constants.OFFLine_Code://断线通知
                            NetBean.OffLineResponseBean offLineResponseBean = GsonUtil.GsonToBean(json, NetBean.OffLineResponseBean.class);
                            Toast.makeText(VideoActivity.this, offLineResponseBean.message, Toast.LENGTH_SHORT).show();
                            break;
                        case Constants.OnLineUserInfo_Code://登录在线人源信息通知
...
//            }
//        });
        NetBean.LoginRequsetBean loginRequsetBean = new NetBean.LoginRequsetBean(Constants.Login_Code, System.currentTimeMillis() + "", FromId);
        RtcWebSokcetHelper.getInstance().putWebsocketDealEvent(GsonUtil.BeanToJson(loginRequsetBean), new RtcWebSokcetHelper.Runnable() {
            ...
            public void run() {
                ...
                if (loginResponseBean.isSucceed) {
                    Toast.makeText(VideoActivity.this, "登录成功", Toast.LENGTH_SHORT).show();
                } else {
                    Toast.makeText(VideoActivity.this, "登录失败，" + loginResponseBean.message, Toast.LENGTH_SHORT).show();
                }
            }
        });
...
////                    Toast.makeText(VideoActivity.this, "请先输入登录UserId", Toast.LENGTH_SHORT).show();
////                    return;
////                }
//
//            }
...
//                    Toast.makeText(VideoActivity.this, "请先填写fromUserId,toUserId", Toast.LENGTH_SHORT).show();
//                    return;
//                }
                String message = GsonUtil.GsonToString(new NetBean.NormalMessage(Constants.Message_Call_Code));
                NetBean.SendMsgRequestBean sendMsgRequestBean = new NetBean.SendMsgRequestBean(Constants.SendMsg_Code, System.currentTimeMillis() + "", fromUserId, toUserId, message);
                RtcWebSokcetHelper.getInstance().putWebsocketDealEvent(GsonUtil.BeanToJson(sendMsgRequestBean), new RtcWebSokcetHelper.Runnable() {
                    ...
                    public void run() {
                        ...
                        if (sendMsgResponseBean.isSucceed) {
                            Log.e(TAG, "run: 呼叫成功");
                            Toast.makeText(VideoActivity.this, "呼叫成功", Toast.LENGTH_SHORT).show();
                        } else {
                            Log.e(TAG, "run: 呼叫失败");
                            Toast.makeText(VideoActivity.this, "呼叫失败，" + sendMsgResponseBean.message, Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    ...
    private void call(String fromUserId, String toUserId) {
        ...
        callPeerConnection = peerConnectionFactory.createPeerConnection(iceServers, new PeerConnectionAdapter("localconnection") {
            ...
            //todo---WS转发
                ...
                String message = GsonUtil.GsonToString(new NetBean.IceCandidateMessage(Constants.Message_IceCandidate_Request_Code, iceCandidate.sdpMid, iceCandidate.sdpMLineIndex, iceCandidate.sdp));
                NetBean.SendMsgRequestBean sendMsgRequestBean = new NetBean.SendMsgRequestBean(Constants.SendMsg_Code, System.currentTimeMillis() + "", fromUserId, toUserId, message);
                RtcWebSokcetHelper.getInstance().sendRequest(GsonUtil.BeanToJson(sendMsgRequestBean));
            }
        ...
        callPeerConnection.createOffer(new SdpAdapter("local offer sdp") {//发送offer
            ...
            public void onCreateSuccess(SessionDescription sessionDescription) {
                ...
                String message = GsonUtil.GsonToString(new NetBean.SdpMessage(Constants.Message_SDP_Offer_Code, sessionDescription.description));
                NetBean.SendMsgRequestBean sendMsgRequestBean = new NetBean.SendMsgRequestBean(Constants.SendMsg_Code, System.currentTimeMillis() + "", fromUserId, toUserId, message);
                RtcWebSokcetHelper.getInstance().sendRequest(GsonUtil.BeanToJson(sendMsgRequestBean));
            }
...
//                String sdp; //todo---WS转发
                ...
                String message = GsonUtil.GsonToString(new NetBean.IceCandidateMessage(Constants.Message_IceCandidate_Response_Code, iceCandidate.sdpMid, iceCandidate.sdpMLineIndex, iceCandidate.sdp));
                NetBean.SendMsgRequestBean sendMsgRequestBean = new NetBean.SendMsgRequestBean(Constants.SendMsg_Code, System.currentTimeMillis() + "", toUserId, fromUserId, message);
                RtcWebSokcetHelper.getInstance().sendRequest(GsonUtil.BeanToJson(sendMsgRequestBean));
        ...
        receivePeerConnection.createAnswer(new SdpAdapter("remote answer sdp") {//发送sdp
            ...
            public void onCreateSuccess(SessionDescription sdp) {
                ...
                String message = GsonUtil.GsonToString(new NetBean.SdpMessage(Constants.Message_SDP_Answer_Code, sdp.description));
                NetBean.SendMsgRequestBean sendMsgRequestBean = new NetBean.SendMsgRequestBean(Constants.SendMsg_Code, System.currentTimeMillis() + "", toUserId, fromUserId, message);
                RtcWebSokcetHelper.getInstance().sendRequest(GsonUtil.BeanToJson(sendMsgRequestBean));
            }
        }, new MediaConstraints());
