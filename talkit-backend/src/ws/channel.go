...
func (channelMap *ChannelMap) get(id uint64) (chan Msg, bool) {
...
func (channelMap *ChannelMap) online(id uint64) chan Msg {
	channel := make(chan Msg, 32)
...
// offline 方法用于从映射表中删除指定ID的通道。它接受一个 id 参数，该参数是要删除的通道的ID。类似于 online 方法，它通过调用 Lock() 方法对 mutex 进行写锁定，以确保安全地删除通道。然后，它使用 delete() 函数从 innerMap 中删除指定ID的通道。最后，它释放写锁定。
func (channelMap *ChannelMap) offline(id uint64) {
	channelMap.mutex.Lock()
...
func NewChannelMap() ChannelMap {
	return ChannelMap{
		innerMap: make(map[uint64]chan Msg, 64),
		mutex:    sync.RWMutex{},
	}
}
